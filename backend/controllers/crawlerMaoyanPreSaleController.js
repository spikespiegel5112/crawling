...
const crawlMovieList = async (req, res, next) => {
  _crawlMovieListPromise(req, res, next)
    .then((response) => {
      res.status(200).json({
        data: response,
      });
...
const crawlPreSaleDetail = async (req, res, next) => {
  _crawlPreSaleDetailPromise(req, res, next)
    .then((response) => {
      console.log(
        "crawlPreSaleDetail+++++++++++++++++++++++++++++++",
        response
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const rawData = {
        ...
        platformType: "Web",
        numWantToSee: $(".movie-baseinfo .block-wish-item.left h2")
          .text()
          .replace("想看", "")
          .trim(),
        byGenderMale: find(
          $,
          ".movie-baseinfo .block-wish-detail p:eq(0)"
        ).text(),
        byGenderFemale: find(
          $,
          ".movie-baseinfo .block-wish-detail p:eq(0)"
      ...
      const result = {
        ...
        byGenderMale: rawData.byGenderMale.match(
          /[1-9]\d*\.\d*|0\.\d*[1-9]\d*$/
        )
          ? rawData.byGenderMale.match(/[1-9]\d*\.\d*|0\.\d*[1-9]\d*$/) + "%"
          : "",
        byGenderFemale: rawData.byGenderFemale.match(
          /[[1-9]\d*\.\d*|0\.\d*[1-9]\d*]$/g
        )
          ? String(
              rawData.byGenderFemale.match(/[[1-9]\d*\.\d*|0\.\d*[1-9]\d*]$/g)
            ).split(",")[1] + "%"
          : "",
      };
...
const crawlPreSaleWantToSeePortrait = async (req, res, next) => {
  return new Promise((resolve, reject) => {
    _crawlPreSaleWantToSeePortraitPromise(req, res, next)
      .then((response) => {
        console.log(
          "_crawlPreSaleWantToSeePortrait+++++++++++++++++++++++++++++++",
          response
        );
        res.status(200).json({
          data: response,
...
const _crawlPreSaleWantToSeePortraitPromise = (req, res, next) => {
  return new Promise(async (resolve, reject) => {
    ...
    req.query = Object.assign(req.query, {
      address: encodeURI(
        req.query.address + "/wantindex?city_tier=0&city_id=0&cityName=全国"
      ),
...
const crawlPreSaleBoxOfficePremiere = async (req, res, next) => {
  _crawlPreSaleBoxOfficePremierePromise(req, res, next)
    .then((response) => {
      console.log(
...
const oneKeyMoviePreSale = async (req, res, next) => {
  ...
  const loop = async () => {
    ...
    let movieId = item2.replace(/[^0-9]/gi, "");

    let addressDetail = "https://piaofang.maoyan.com/movie/" + movieId;
    let addressPreSale = encodeURI(
      "https://piaofang.maoyan.com/movie/" +
        movieId +
        "/wantindex?city_tier=0&city_id=0&cityName=全国"
    );
    ...
    try {
      dataWantDetail = await _crawlPreSaleDetailPromise(
        Object.assign(req, queryPreSaleList),
        res,
        next
      );
      ...
      dataPreSalePortrait = await _crawlPreSaleWantToSeePortraitPromise(
        Object.assign(req, queryPreSalePortrait),
        res,
        next
      );
...
const crawlPreSaleBookingDetails = async (req, res, next) => {
  return new Promise((resolve, reject) => {
    _crawlPreSaleBookingDetailsPromise(req, res, next)
      .then((response) => {
        console.log(
          "_crawlPreSaleBookingDetailsPromise+++++++++++++++++++++++++++++++",
          response
        );
        res.status(200).json({
          data: response,
...
const _crawlPreSaleBookingDetailsPromise = (req, res, next) => {
  return new Promise(async (resolve, reject) => {
    ...
    const address =
      "https://piaofang.maoyan.com/movie/" + movieId + "/premierebox";
    req.query = Object.assign(req.query, {
    ...
    try {
      ...
      // const base64 = $('#js-nuwa').html().match(/(?<=src:url\().+.(?=\)\sformat\("woff"\))/)[0];
      const bookingDateListLength = $(
        ".section-detail .t-table .t-main-col .t-row"
      ).length;
      const bookingDateList = [];

      for (let i = 0; i < bookingDateListLength; i++) {
        bookingDateList.push(
          find($, ".section-detail .t-table .t-main-col .t-row:eq(" + i + ")")
            .text()
            .trim()
        );
      }
      ...
      bookingDateList.forEach((item, index) => {
        rawData.push({
          ...
          bookingDate: find(
            $,
            ".section-detail .t-table .t-main-col .t-row:eq(" + index + ")"
          )
            .text()
            .replace(/[\u4e00-\u9fa5]+/, "")
            .trim(),
          accumulatedFirstDayPreSale: find(
            $,
            ".section-detail .t-table .t-other-col .t-row:eq(" +
              index +
              ") .t-col:eq(0)"
          )
            .text()
            .trim(),
          dailyAdditionalPreSale: find(
            $,
            ".section-detail .t-table .t-other-col .t-row:eq(" +
              index +
              ") .t-col:eq(1)"
...
const saveMultipleMaoyanPreSaleBookingDetails = (req, res, next) => {
  ...
  _saveMultipleMaoyanPreSaleBookingDetailsPromise(req, res, _timestamp)
    .then((response) => {
      res.status(200).json({
        message: "success",
        data: response,
...
const _saveMultipleMaoyanPreSaleBookingDetailsPromise = (
  req,
  res,
  _timestamp
) => {
      ...
      return new Promise(async (resolve, reject) => {
        const loop = async () => {
          if (bookingDetailsCount < requestBody.length) {
            ...
            console.log(
              "requestBody[bookingDetailsCount]+++++++++",
              requestBody[bookingDetailsCount]
            );

            await MaoyanPreSaleBookingDetailsModel.bulkCreate(
              requestBody[bookingDetailsCount]
            );
            bookingDetailsCount++;
            loop();
...
const save = (req, res, next) => {
  ...
  _createRecordPromise(req, timestamp)
    .then((response) => {
      res.status(200).json({
        data: req,
      });
...
const saveOneMaoyanPreSale = (req, res, next) => {
  ...
  _createMaoyanPreSaleRecord(req, timestamp)
    .then((response) => {
      res.status(200).json({
        message: "success",
        data: response,
...
const saveMultipleMaoyanPreSale = (req, res, next) => {
  ...
  _createMultipleMaoyanPreSaleRecordPromise(requestBody)
    .then((response) => {
      res.status(200).json({
        data: response,
      });
...
const _createMultipleMaoyanPreSaleRecordPromise = (requestBody) => {
  return new Promise((resolve, reject) => {
    MaoyanPreSaleModel.bulkCreate(requestBody)
      .then((result) => {
        // console.log(result);
        resolve(result);
      })
...
const deleteRecords = (req, res, next) => {
  ...
  let selections =
    req.body.selections instanceof Array
      ? req.body.selections
      : [req.body.selections];
  const Op = Sequelize.Op;
  const deleteRecord = () => {
  ...
  Promise.all([
    deleteRecordPromise,
    deleteBookingDetailsPromise,
    deleteBookingDetailsMappingPromise,
  ])
    .then((responseAll) => {
      res.status(200).json({
        message: "success",
    ...
    return new Promise((resolve, reject) => {
      if (idSelections instanceof Array) {
        idSelections.forEach((item, index) => {
          MaoyanPreSaleModel.findByPk(item)
            .then(async (response) => {
        ...
        MaoyanPreSaleModel.findByPk(idSelections)
          .then((result) => {
            console.log(result);
            result.destroy().then(() => {
              if (index + 1 === req.body.id.length) {
...
exports.saveMultipleMaoyanPreSaleBookingDetails =
  saveMultipleMaoyanPreSaleBookingDetails;
