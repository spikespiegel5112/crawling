  ...
  <el-row class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="抓取时间" prop="timestamp" width="100">
        <template
          slot-scope="scope"
        >{{$moment(Number(scope.row.timestamp)).format('YYYY-MM-DD HH:mm:ss')}}</template>
      </el-table-column>
      ...
      <el-table-column align="center" label="描述" prop="description" width="100">
    ...
    >
      <el-row :gutter="1">
        <el-col :span="12">
          <el-button
            @click="prepareToCrawlFlag=true"
            type="primary"
          >{{rankingListData.length===0?'获取索引':'重新获取索引'}}</el-button>
          <el-button
            :disabled="rankingListData.length===0"
            @click="beginToCrawRankingListMovieData"
            type="primary"
            v-if="!crawlingFlag"
          >{{crawlingCount===0?'开始抓取':'重新抓取'}}</el-button>
              ...
              <el-card shadow="never">
                ...
                <el-divider>
                  <el-row>
                    <el-col
                      :span="24"
                      v-if="rankingListData.length>0&&crawlingCount<rankingListData.length"
                    >共有{{rankingListData.length}}条数据，正在抓取第{{crawlingCount}}条...</el-col>
                    <el-col :span="24" v-else>共有{{rankingListData.length}}条数据，抓取完毕</el-col>
                  </el-row>
                </el-divider>
                <el-row justify="left" type="flex">
                  <el-col :span="24">
                        ...
                        <el-card shadow="hover">
                          <el-row>
                            ...
                            <el-col :span="1">
                              ...
                              <i class="el-icon-close failed" v-else-if="item.detailSuccess===2"></i>
                            </el-col>
                            <el-col :span="1">
                              更多:
                              <i class v-if="item.moreSectionsSuccess===0"></i>
                              <i
                            ...
                            <el-col :span="1">
                              评分:
                              <i class v-if="item.rankingListRatingSuccess===0"></i>
                              <i
                                class="el-icon-check success"
                              ...
                              <i
                                class="el-icon-close failed"
                                v-else-if="item.rankingListRatingSuccess===2"
                              ></i>
                            </el-col>

                            <el-col :span="1">
                              想看:
                              <i class v-if="item.rankingListWantToSeePortraitSuccess===0"></i>
                              <i
                                class="el-icon-check success"
                                v-else-if="item.rankingListWantToSeePortraitSuccess===1"
                              ></i>
                              <i
                                class="el-icon-close failed"
                                v-else-if="item.rankingListWantToSeePortraitSuccess===2"
                              ></i>
                            </el-col>
                            <el-col :span="2">
                              首映日票房:
                            ...
                            <el-col :span="2">
                              全球票房:
                              <i class v-if="item.rankingListBoxOfficeGlobalSuccess===0"></i>
                              <i
                                class="el-icon-check success"
                                v-else-if="item.rankingListBoxOfficeGlobalSuccess===1"
                              ></i>
                              <i
                                class="el-icon-close failed"
                                v-else-if="item.rankingListBoxOfficeGlobalSuccess===2"
                              ></i>
                            </el-col>
                          </el-row>
                        </el-card>
    ...
    <el-dialog :visible.sync="prepareToCrawlFlag" title="准备爬取" width="600px">
      <el-radio-group @change="handleChangeLimitType" v-model="crawlerCountType">
        ...
        <el-input-number
          ...
          @keyup.enter.native="getAllCrawlingIndex"
          v-model="rankingListCountLimit"
        ></el-input-number>
      </el-radio-group>

      <el-button
        @click="getAllCrawlingIndex"
        type="primary"
        :disabled="fetchingCrawlingIndexFlag"
      >开始抓取列表</el-button>
    </el-dialog>
  </el-row>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      exportCSVRequest: 'common/exportCSV',
      ...
      fetchingCrawlingIndexFlag: false,
      crawlerCountType: 0,
  ...
  methods: {
    ...
    async getAllCrawlingIndex() {
      this.fetchingCrawlingIndexFlag = true
      await this.getRankingList()
      this.fetchingCrawlingIndexFlag = false
    ...
    getMovieData() {
      const getDetailData = this.getDetailDataPromise()
        ...
        const getRankingListWantToSeePortrait = () => {
          return new Promise((resolve, reject) => {
            this.$http.get(this.$baseUrl + this.crawlRankingListWantToSeePortraitRequest, {
              params: {
                address: 'https://piaofang.maoyan.com/movie/' + movieId + '/wantindex??city_tier=0&city_id=0&cityName=全国',
                headerCode: 'rankingListDetailHeader'
              },
              timeout: this.requestTimeOut
            }).then(response1 => {
    ...
    async beginToCrawlPreSale() {
      ...
      const loop = () => {
        ...
        if (this.crawlingCount === this.rankingListCountLimit - 1) {
          // debugger
          this.crawlingFlag = false
        } else {
          this.crawlingCount++
    ...
    handleExportCSV() {
      this.$http.post(this.$baseUrl + this.exportCSVRequest, {
        data: this.tableList
      }).then(response => {
        console.log(response)
      }).catch(error => {
